# Staffing Scheduling Playground

Thin browser evaluator for `staffing-scheduling`.
...
```bash
pnpm --filter staffing-scheduling-playground dev
```
...
```bash
pnpm --filter staffing-scheduling-playground test:e2e
```
...
- It uses the current `staffing-scheduling` package as the authority for compile, solve, and regroup behavior.
